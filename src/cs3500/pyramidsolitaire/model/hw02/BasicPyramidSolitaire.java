...
/**
 * Basic pyramid solitaire implementation of model, this model computes the moves of the game and
 * gives the information to the controller.
 */
public class BasicPyramidSolitaire implements PyramidSolitaireModel<Card> {
  private int numRows;
  private int pyramidCardNum;
  private int numDraw;
  private final List<Card> stockCards;
  private final List<ArrayList<Card>> pyramid;
  private final List<ArrayList<Boolean>> coveredCards;
  private final List<Card> drawCards;
  private boolean gameStarted;
  private int score;

  /** Constructor for the class initializes the values to their starting states. */
   ...
   */
  private void newExposed(int row, int card) {
    if (row > 0) {
   ...
   */
  private void checkCard(int row, int card) {
    if ((row > (this.numRows - 1)) || (card > row) || card < 0) {
   ...
   */
  private boolean isNotNull(int row, int card) {
