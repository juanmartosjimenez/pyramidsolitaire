...
public interface PyramidSolitaireModel<K> {
   ...
   *
   * <p>This method first verifies that the deck is valid. It deals cards in rows (left-to-right,
   * top-to-bottom) into the characteristic pyramid shape with the specified number of rows,
   * followed by the specified number of draw cards. When {@code shuffle} is {@code false}, the 0th
   * card in {@code deck} is used as the first card dealt.
   *
   * <p>This method should have no other side effects, and should work for any valid arguments.
   *
   ...
   *     number of available draw cards is non-positive, or a full pyramid and draw pile cannot be
   *     dealt with the number of given cards in deck
   */
